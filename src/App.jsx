...
function App() {
  ...
  useEffect(() => {
    (async function () {
      const urls = [
        "https://api.themoviedb.org/3/movie/now_playing?language=vi&page=10",
        "https://api.themoviedb.org/3/movie/top_rated?language=vi&page=10",
        // Add more URLs here...
      ];
      const options = {
        method: "GET",
    ...
    <>
      <MovieProvider>
        <div className="h-full bg-black text-white min-h-screen pb-10 relative">
          ...
          <Banner />
          {searchData.length === 0 && (
            <MovieList title="Phim Hot" data={trendingMovies.slice(0, 20)} />
          )}
          {searchData.length === 0 && (
            <MovieList title="Phim đề cử" data={topRatedMovies.slice(0, 20)} />
          )}
